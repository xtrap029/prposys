...
class TransactionsFormsController extends Controller {
    ...
    public function create() {
        ...
        if ($transaction->is_deposit)
            ...
            $page_title = config('global.trans_category_label_make_form')[10];
        else
    ...
    public function edit(Transaction $transaction) {
        ...
        if ($transaction->is_deposit)
            ...
            $page_title = config('global.trans_category_label_edit_form')[10];
        else
    ...
    public function update(Request $request, Transaction $transaction) {
        ...
        } else if ($data['trans_category'] == config('global.trans_category')[10]) {
    ...
    public function issue(Request $request, Transaction $transaction) {
        if ($this->check_can_issue($transaction->id)) {
            ...
            if ($transaction->trans_type == "po" && ($transaction->is_tdsa_payment || $transaction->is_aec_payment || $transaction->is_aff_advances) && $transaction->src_transaction_id) {
    ...
    public function reassign_approver(Request $request, Transaction $transaction) {
        ...
        $transaction->form_assigned_approver_id = $request->user_id;
        $transaction->save();
    ...
    public function clear_reassigned_approver(Transaction $transaction) {
        ...
        $transaction->form_assigned_approver_id = null;
        $transaction->save();
    ...
    private function check_can_reassign_approver($transaction) {
        ...
        if (UAHelper::get()['form_assign_approver'] == config('global.ua_none')) {
            return false;
        }
