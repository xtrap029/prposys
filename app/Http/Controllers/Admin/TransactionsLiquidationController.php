...
class TransactionsLiquidationController extends Controller {
    ...
    public function index($trans_page, $trans_company = '') {
        ...
        foreach ($transactions as $key => $value) {
            $transactions[$key]->can_edit = TransactionHelper::check_can_liquidation_edit($value->id);
            $transactions[$key]->can_reset = TransactionHelper::check_can_liquidation_reset($value->id);
            $transactions[$key]->can_approval = TransactionHelper::check_can_liquidation_approval($value->id);
            $transactions[$key]->can_print = TransactionHelper::check_can_liquidation_print($value->id);
            $transactions[$key]->can_clear = TransactionHelper::check_can_liquidation_clear($value->id);
    ...
    public function create() {
        if (empty($_GET['key']) || !TransactionHelper::check_can_liquidation_create($_GET['key'])) {
    ...
    public function store(Request $request) {
        if (!TransactionHelper::check_can_liquidation_create($request->key)) {
    ...
    public function show(Transaction $transaction) {
        ...
        $perms['can_edit'] = TransactionHelper::check_can_liquidation_edit($transaction->id);
        $perms['can_reset'] = TransactionHelper::check_can_liquidation_reset($transaction->id);
        $perms['can_approval'] = TransactionHelper::check_can_liquidation_approval($transaction->id);
        $perms['can_print'] = TransactionHelper::check_can_liquidation_print($transaction->id);
        $perms['can_clear'] = TransactionHelper::check_can_liquidation_clear($transaction->id);
        $perms['can_edit_cleared'] = TransactionHelper::check_can_liquidation_clear_edit($transaction->id);
    ...
    public function edit(Transaction $transaction) {
        if (!TransactionHelper::check_can_liquidation_edit($transaction->id)) {
    ...
    public function approval(Transaction $transaction) {
        if (TransactionHelper::check_can_liquidation_approval($transaction->id)) {
            // $data = $request->validate([
    ...
    public function print (Transaction $transaction) {
        if (!TransactionHelper::check_can_liquidation_print($transaction->id)) {
    ...
    public function clear(Request $request, Transaction $transaction) {
        if (TransactionHelper::check_can_liquidation_clear($transaction->id)) {
            $data = $request->validate([
