...
class TransactionsLiquidationController extends Controller {
    ...
    public function create() {
        ...
        else if ($transaction->is_aff_advances)
            $page_title = config('global.trans_category_label_create_liq')[10].' '.strtoupper($transaction->trans_type);
        else
    ...
    public function show(Transaction $transaction) {
        ...
        if ($transaction->is_tdsa_bill || $transaction->is_aec_bill || ($transaction->is_aff_advances && $transaction->trans_type == 'pr')) {
        ...
        if ($this->check_can_clear($transaction->id)) {
            ...
            if ($transaction->is_tdsa_bill || $transaction->is_aec_bill || ($transaction->is_aff_advances && $transaction->trans_type == 'pr')) {
                ...
                if ($autogenerated_transaction) {
                    $autogenerated_transaction->update([
                        'depo_type' => $transaction->depo_type,
                        'depo_bank_branch_id' => $transaction->depo_bank_branch_id,
                        'depo_ref' => $transaction->depo_ref,
                        'depo_received_by' => $transaction->depo_received_by,
                        'depo_date' => $transaction->depo_date,
                        'depo_slip' => $transaction->depo_slip,
                        'status_updated_at' => $transaction->status_updated_at,
                        'status_prev_id' => $autogenerated_transaction->status_id,
                        'status_id' => 9,
                        'liquidation_approver_id' => $transaction->liquidation_approver_id,
                        'updated_id' => $transaction->updated_id
                    ]);

                    $transaction_liquidation = TransactionsLiquidation::where('transaction_id', $transaction->id)->get();
                    foreach ($transaction_liquidation as $value) {
                        $liquidation = $value->replicate();
                        $liquidation->transaction_id = $autogenerated_transaction->id;
                        $liquidation->save();
                    }

                    $transaction_attachments = TransactionsAttachment::where('transaction_id', $transaction->id)->get();
                    foreach ($transaction_attachments as $value) {
                        $attachment = $value->replicate();
                        $attachment->transaction_id = $autogenerated_transaction->id;
                        $attachment->save();
                    }
                }
            }
    ...
    public function print_cleared() {
        ...
        foreach ($transactions as $key => $transaction) {
            ...
            if ($transaction->is_tdsa_bill || $transaction->is_aec_bill || ($transaction->is_aff_advances && $transaction->trans_type == 'pr')) {
                $autogenerated_transaction = Transaction::where('src_transaction_id', $transaction->id)->first();
                $transactions[$key]->autogenerated_transaction = $autogenerated_transaction;
            }
        }
    ...
    public function reassign_approver(Request $request, Transaction $transaction) {
        if (!$this->check_can_reassign_approver($transaction->id)) {
            return back()->with('error', __('messages.cant_edit'));
        }

        $request->validate(['user_id' => 'required|exists:users,id']);

        $transaction->liq_assigned_approver_id = $request->user_id;
        $transaction->save();

        return back()->with('success', 'Approver reassigned successfully.');
    }

    public function clear_reassigned_approver(Transaction $transaction) {
        if (!$this->check_can_reassign_approver($transaction->id)) {
            return back()->with('error', __('messages.cant_edit'));
        }

        $transaction->liq_assigned_approver_id = null;
        $transaction->save();

        return back()->with('success', 'Reassigned approver cleared.');
    }
    ...
    private function check_can_reassign_approver($transaction) {
        ...
        if (UAHelper::get()['liq_assign_approver'] == config('global.ua_none')) {
            return false;
        }
