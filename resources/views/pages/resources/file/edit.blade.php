...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            <form action="/files-manage/{{ $file->id }}" method="post">
                @csrf
                @method('put')
                <div class="form-row">
                    <div class="form-group col-md-8">
                        <label for="">Name</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ $file->name }}"" required>
                        @include('errors.inline', ['message' => $errors->first('name')])
                    </div>
                    <div class="form-group col-md-4">
                        <label for="">Drive ID</label>
                        <input type="text" class="form-control @error('drive') is-invalid @enderror" name="drive" value="{{ $file->drive }}"" required>
                ...
                <div class="form-group">
                    ...
                    <select name="ua_level_ids[]" class="form-control form-control-sm chosen-select" multiple>
                        @foreach ($ua_levels as $item)
                            <option value="{{ $item->id }}" {{ in_array($item->id, $ua_level_ids) ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
