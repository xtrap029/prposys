...
Route::group(['middleware' => ['auth', 'CheckUserAccess:active', 'CheckConfidential']], function () {
    ...
    // Peo User Hierarchy
    Route::prefix('people-hierarchy')->group(function () {
        $url = 'People\HierarchyController';

        Route::middleware('CheckUserAccess:peo_hierarchy')->group(function () use ($url) {
            Route::get('/', $url . '@index')->name('people-hierarchy');
        });

        Route::middleware('CheckUserAccess:peo_hierarchy_manage')->group(function () use ($url) {
